...
class task_widget(QWidget):
    def __init__(self,parent=None):
        ...
        self.regular_task_label = True

    def _sorted(self, keys):
        #key either look like Task 1 or any else defined by user
        if re.match(r'\bTask \d\b',list(keys)[0]):
            self.regular_task_label = True
    ...
    def passed_task(self, key):
        ...
        if self.regular_task_label:
            ...
            if list(self.task_info.keys()).index(self.current_task[0]) > list(self.task_info.keys()).index(key):
                return True
            else:
                return False
