...
class MyMainWindow(QMainWindow):
    def __init__(self, parent = None):
        ...
        ui_path = str(Path(__file__).parent.parent/ "resources" / "ui" / "task_manager.ui")
        uic.loadUi(ui_path,self)
        self.setWindowTitle('Task manager: make your daily tasks flow as time')
        self.lineEdit_task_date.setText(datetime.datetime.now().strftime("%Y-%m-%d"))
    ...
    def load_task_files(self):
        ...
        self.comboBox_task_files.addItems(os.listdir(str(Path(__file__).parent.parent/ "resources" / "task_files")))
    ...
    def init_task_table(self):
        ...
        ends = []
        dates = []
        if not re.match(r'\b\d{4}-\d{2}-\d{2}\b',self.lineEdit_task_date.text()):
            error_pop_up('The date format is not corret. Please use yyyy-mm-dd format!')
            return
        for i in range(self.spinBox_task_number.value()):
            ...
            begins.append('08:00')
            ends.append('12:00')
            dates.append(self.lineEdit_task_date.text())
    ...
    def save_task_table(self):
        ...
        lines = []
        num_of_default_files = len([each for each in Path('./task_files').iterdir() if each.match('*_default*')])
        if daily:
            file = 'task_file_daily_default_{}'.format(num_of_default_files+1)
        else:
            file = 'task_file_{}'.format(self.lineEdit_task_date.text())
        #with open(os.path.join(script_path,'task_files',file),'w') as f:
        with open(str(Path(__file__).parent.parent/ "resources" / "task_files" / file), 'w') as f:
            for i in range(len(self.pandas_model._data.index)):
                ...
                lines.append("{}+['{}','{}','{}']".format(*([self.pandas_model._data.loc[i]['task_label']]+self.pandas_model._data.loc[i][['begin_time','end_time','note']].tolist())))
            f.write('\n'.join(lines))
        self.load_task_files()
    ...
    def init_task_file(self):
        ...
        files = os.listdir(str(Path(__file__).parent.parent/ "resources" / "task_files"))
        file = None
        for each in files:
    ...
    def set_task_file(self):
        ...
        file = str(Path(__file__).parent.parent/ "resources" / "task_files" / self.comboBox_task_files.currentText())
...
def main():
    #ui_file = str(Path(__file__).parent.parent/ "resources" / "ui" / "task_manager.ui")
